...
So, supporting dark mode means, now there are two different themes&mdash;say, _light_ and _dark_&mdash;and every time a user changes their theme, that change should also be saved somewhere so that when the user comes back later, the website should remember what theme the user has last selected, and show that theme.
